...
export class AuthController {
    ...
    async register(req: Request, res: Response) {
        ...
        const { email, password, businessName, businessType, contactPhone, businessAddress } = req.body;
        ...
        try {
            ...
            this.emailService.sendWelcomeEmail(merchant.email, merchant.businessName)
            ...
            const response: ApiResponse = {
                ...
                data: {
                    merchant: {
                        id: merchant.id,
                        email: merchant.email,
                        businessNmae: merchant.businessName,
                        status: merchant.status,
                        kycStatus: merchant.kycStatus, 
                    },
                    accessToken,
    ...
    async forgotPassword(req: Request, res: Response) {
        ...
        try {
            ...
            const resetToken = this.authService.generatePasswordResetToken(merchant.id);
    ...
    async resetPassword(req: Request, res: Response) {
        ...
        try {
            const isValid = await this.authService.resetPassword(token, password);
            if (!isValid) {
                return res.status(400).json({
                    success: false,

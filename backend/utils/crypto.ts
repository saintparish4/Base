...
const { algorithm, keyLength, ivLength, saltLength, tagLength } = config.encryption;
...
// DERIVE KET FROM PASSWORD USING PBKDF2
export function deriveKey(password: string, salt: Buffer, iterations = 100000): Buffer {
...
export function decrypt(encryptedData: { encrypted: string; iv: string; tag: string }, key: Buffer): string {
    const { encrypted, iv, tag } = encryptedData;

    const decipher = crypto.createDecipheriv(algorithm, key, Buffer.from(iv, 'hex')) as crypto.DecipherGCM;
